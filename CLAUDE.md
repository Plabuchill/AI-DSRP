...
- **เปิดดู prototype**: `npx http-server prototypes/v1 -p 8743 -c-1` แล้วเข้า `http://localhost:8743` (หรือเปิด `prototypes/v1/index.html` ตรงๆ ในเบราว์เซอร์ก็ได้ แต่บาง path relative อาจไม่ครบ) — หรือเข้า production ตรงที่ `https://ai-dsrp.web.app` (Firebase Hosting)
- **ติดตั้ง dependency ของสคริปต์เสริม**: `npm install` ที่ root (มีแค่ `firebase-admin` เป็น devDependency สำหรับ `scripts/seed/`) — `functions/` มี `package.json`/dependency แยกต่างหาก ต้อง `cd functions && npm install` เอง
...
- **Deploy Hosting + Firestore Rules**: `firebase deploy --only hosting,firestore:rules` (ต้อง `firebase login` ด้วยบัญชีที่มีสิทธิ์เข้าโปรเจกต์ `ai-dsrp` ก่อน)
- **Deploy Cloud Functions**: ต้องตั้ง secret ก่อนครั้งแรก (หรือทุกครั้งที่หมุนเวียนคีย์) ด้วย `firebase functions:secrets:set ANTHROPIC_API_KEY` (ต้องพิมพ์ค่าเองในเครื่อง ห้ามให้ Claude Code กรอกแทน — เป็น credential) แล้ว `firebase deploy --only functions` — ต้องใช้ Firebase plan **Blaze (pay-as-you-go)** เท่านั้น เพราะ Cloud Functions เรียก API ภายนอก (Anthropic) ไม่ได้ถ้าอยู่ Spark

## 8. Code Architecture

- **`prototypes/v1/`** — 13 หน้า HTML แยกอิสระ: **8 หน้าโมดูลหลัก mock ล้วน** (`index`, `case-intake`, `case-analysis`, `control-plan`, `field-tracking`, `asm-coordination`, `reports`, `alerts`) + **5 หน้าที่เชื่อม Firebase จริง** (`login`, `signup`, `user-approval`, `new-506-request`, `506-request-detail`) ใช้ `styles.css` ร่วมกัน และ left-rail navigation ที่ copy-paste ซ้ำทุกหน้า (ยังไม่มี templating/partial ตามที่ `TECH-STACK.md` วางแผนไว้) แต่ละหน้าคู่กับไฟล์ `<page>.js` แบบ classic script (IIFE) ที่ฝัง mock data array ไว้ในตัว
- **ไฟล์ที่เชื่อม Firebase จริง (ทุกไฟล์เป็น `type="module"`)**: `firebase-init.js` (จุดเดียวที่ประกาศ config, export `app`/`db`/`auth`/`functions`), `auth-guard.js` (โหลดทุกหน้ายกเว้น `login.html` — เช็ก login + เติมชื่อ/role ที่ left rail + route guard ตาม role ดู `ACL.md`), `current-user.js` (module กลาง resolve role ปัจจุบัน), `login.js`/`signup.js`/`user-approval.js` (Firebase Authentication), `case-analysis-506.js`/`new-506-request.js`/`506-request-detail.js` (Firestore จริงสำหรับ "บันทึกและยืนยัน รง.506" `FEAT-ANALYSIS-07`), `case-intake-upload.js` (เรียก Cloud Function `extractCaseReport` สำหรับ OCR จริง `FEAT-INTAKE-05` — ผลลัพธ์ส่งกลับให้ `case-intake.js` ผ่าน `CustomEvent` "case-intake:ocr-result" แทนที่จะ import ตรงๆ เพราะ `case-intake.js` เป็น classic script ไม่ใช่ module) — หน้าที่เชื่อม Firebase ทุกหน้า**ต้องมีอินเทอร์เน็ต**
- **`functions/`** — Firebase Cloud Functions v2 (Node.js 20) แยก `package.json`/dependency ของตัวเองจาก root — ปัจจุบันมีฟังก์ชันเดียวคือ `extractCaseReport` (callable, เช็ก `request.auth` ก่อนเสมอ) เรียก Claude Vision (Anthropic SDK) อ่านไฟล์ PDF/JPEG/PNG ที่อัปโหลดจาก Case Intake แล้วคืนข้อมูลโครงสร้างผ่าน forced tool-use — ต้องมี secret `ANTHROPIC_API_KEY` ตั้งไว้ก่อน deploy (ดูข้อ 7) และ project ต้องอยู่ Firebase plan Blaze — ผลลัพธ์ OCR ยังเก็บใน memory ของหน้า Case Intake เท่านั้น (ไม่ persist ลง Firestore ในรอบนี้ ต่างจาก รง.506)
...
- **`firebase.json`/`.firebaserc`/`firestore.rules`** — config การ deploy จริง (Hosting ชี้ที่ `prototypes/v1`, Firestore Rules บังคับ `request.auth != null` ทุก collection, Functions ชี้ที่ `functions/`)
...
- **เอกสารคือ source of truth ของ scope/สถาปัตยกรรมที่ตั้งใจไว้** ไม่ใช่โค้ด — โค้ดปัจจุบันยังตามหลัง `ROADMAP.md`/`FEATURE-LIST.md`/`DATA-MODEL.md`/`TECH-STACK.md` อยู่มาก (8 โมดูลหลักยังเป็น mock ทั้งหมด มีแค่ระบบ Login/สิทธิ์ผู้ใช้, รง.506, และ OCR ของ Case Intake เท่านั้นที่ต่อ backend/AI จริงแล้ว) ก่อนเพิ่มฟีเจอร์ใหม่ในโค้ดต้องเช็กเอกสารเหล่านี้ก่อนเสมอ (ดูข้อ 1-6 ด้านบน) — ห้ามอนุมานสถาปัตยกรรมจากโค้ดที่มีอยู่เพียงอย่างเดียว
